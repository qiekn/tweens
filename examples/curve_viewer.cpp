...
int main() {
  InitWindow(1280, 800, "ck.tween — curve viewer");
  SetWindowState(FLAG_WINDOW_RESIZABLE);
  SetTargetFPS(60);

  constexpr int kCols = 6;
  constexpr int kRows = 6;  // 36 cells, 31 used
  constexpr float kCycleSec = 2.0f;

  const Color kBg = {25, 25, 30, 255};
  const Color kCellFrame = {50, 50, 60, 255};
  const Color kAxisMain = {70, 70, 80, 255};
  const Color kAxisDim = {70, 70, 80, 100};
  const Color kCurve = {120, 200, 255, 255};
  ...
  while (!WindowShouldClose()) {
    ...
    for (std::size_t i = 0; i < kEasings.size(); ++i) {
      ...
      // baseline (y=0) and target line (y=1)
    ...
    DrawText(TextFormat("t = %.2f", t01), 8, win_h - 22, 14, GRAY);
